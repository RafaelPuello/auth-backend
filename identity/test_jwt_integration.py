...
class JWTAuthenticationIntegrationTests(TestCase):
    ...
    def test_login_with_invalid_password_fails(self):
        ...
        response = self.client.post(
            f"{self.base_url}/auth/login",
            data=json.dumps({"email": self.user_email, "password": "WrongPassword123"}),
            content_type="application/json",
        )
    ...
    def test_request_without_token_fails(self):
        ...
        response = self.client.get(f"{self.base_url}/account/email")
    ...
    def test_logout_clears_authentication(self):
        ...
        self.client.delete(f"{self.base_url}/auth/session")
        ...
        fresh_client = Client()
        response = fresh_client.get(f"{self.base_url}/account/email")
    ...
    def test_token_expiration_enforced(self):
        ...
        payload = {"sub": self.user.pk, "exp": exp, "iat": now, "user_id": self.user.pk}
    ...
    def test_me_endpoint_includes_methods_and_flows(self):
        ...
        response = self.client.get(
            "/api/me", HTTP_AUTHORIZATION=f"Bearer {access_token}"
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
...
class JWTSecurityIntegrationTests(TestCase):
    ...
    def test_token_cannot_be_forged(self):
        ...
        response = self.client.get(
            f"{self.base_url}/account/email", HTTP_AUTHORIZATION=f"Bearer {fake_token}"
        )
...
class JWTErrorHandlingIntegrationTests(TestCase):
    ...
    def test_empty_authorization_header_fails(self):
        ...
        response = self.client.get(
            f"{self.base_url}/account/email", HTTP_AUTHORIZATION=""
        )
    ...
    def test_malformed_authorization_header_fails(self):
        ...
        response = self.client.get(
            f"{self.base_url}/account/email", HTTP_AUTHORIZATION="NotBearer token"
        )
    ...
    def test_deleted_user_token_fails(self):
        ...
        response = self.client.get(
            f"{self.base_url}/account/email", HTTP_AUTHORIZATION=f"Bearer {token}"
        )
